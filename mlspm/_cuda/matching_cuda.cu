...
torch::Tensor match_template_cuda(torch::Tensor array, torch::Tensor temp, TMMethod method) {
    ...
    size_t shmem_size = (temp_size + subarray_size) * sizeof(float);

    cudaDeviceProp props;
    cudaGetDeviceProperties(&props, 0);
    if (props.sharedMemPerBlock < shmem_size) {
        throw std::runtime_error("The required shared memory size exceeds device capability.");
    }

    for (int batch_ind = 0; batch_ind < batch_size; batch_ind++) {
        AT_DISPATCH_FLOATING_TYPES(array.type(), "match_template_kernel", ([&] {
            match_template_kernel<scalar_t><<<blocks, threads, shmem_size>>>(
