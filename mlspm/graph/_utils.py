...
def add_rotation_reflection_graph(X, mols, box_borders, num_rotations=1, reflections=True, crop=None,
    per_batch_item=True):
    '''
    ...
    box_center = (
        (box_borders[1][0] + box_borders[0][0]) / 2,
        (box_borders[1][1] + box_borders[0][1]) / 2
    )
        ...
        for k, x in enumerate(X):
            ...
            for i in range(x.shape[0]):
                for j in range(x.shape[-1]):
                    x[i,:,:,j] = np.array(Image.fromarray(x[i,:,:,j]).rotate(rotations[i],
