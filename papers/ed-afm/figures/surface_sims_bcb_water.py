...
if __name__ == "__main__":
    ...
    for ib in range(len(molecules)):
        ...
        # Load data
        X1 = np.load(molecules[ib] / "data_CO_sim.npz")["data"].astype(np.float32)
        X2 = np.load(molecules[ib] / "data_Xe_sim.npz")["data"].astype(np.float32)
        X = [X1[None], X2[None]]
        Y_hartree = [np.load(molecules[ib] / "ESMapHartree.npy")[None]]
