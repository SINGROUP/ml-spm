...
if __name__ == "__main__":
    ...
    save_file       = Path("mse_spring_constants_lat.csv")  # File to save MSE values into
    device          = "cuda"                                # Device to run inference on
    molecules_dir   = Path("../../molecules")               # Path to molecule database
    sim_save_dir    = Path("./test_sim_lat")                # Directory where to save test simulations for inspection
        ...
        for ib, batch in enumerate(trainer):
            ...
            if ib == 0:
                ...
                for s in range(10):
                    ...
                    plt.savefig(sim_save_dir / f"sim{s}_klat_{k_lat:.3f}.png")
                    plt.close()
