...
if __name__ == "__main__":
    ...
    stats_dir = Path("./stats") # Path to directory where csv files for losses are stored
    fig_width = 130             # Figure width in mm
    fontsize  = 8
    dpi       = 300
    linewidth = 1

    with tarfile.open(stats_dir / "stats.tar.gz") as f:
        f.extractall(stats_dir)

    data_lat = np.loadtxt(stats_dir / "mse_spring_constants_lat.csv", delimiter=",")
